...
export class StateMasterDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ stateMaster }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(StateMasterDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
        this.ngbModalRef.componentInstance.stateMaster = stateMaster;
        this.ngbModalRef.result.then(
          result => {
            this.router.navigate(['/state-master', { outlets: { popup: null } }]);
